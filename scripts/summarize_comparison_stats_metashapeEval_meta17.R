...
stats_files = list.files(data("drone_map_evals_meta17/compiled"), pattern="comparison_stats_metashapeEval\\.csv", full.names = TRUE)
...
## Pull in config defs
stats = stats %>%
  mutate(drone_map_name = str_replace(drone_map_name,fixed("lasparamset"),"paramset")) %>% # because of accidentally appending "las" to "paramset" for the las methods
...
comp_neigh_summ %>%
  ungroup() %>%
  filter(metric == "f_score") %>%
  summarize(mean = mean(improvementTo200),
            sd = sd(improvementTo200))
